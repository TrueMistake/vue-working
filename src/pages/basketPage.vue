            ...
            <div class="basketPage-list">
                <div class="basketPage-item" v-for="item in arrToBuy" :key="item.id + Math.random()">
                    ...
                    <div class="basketPage-item__num">
                        <div @click="payment(item,'plus')" class="basketPage-item__num-plus">+</div>
                        <input type="number" :value="item.buy">
                        <div @click="payment(item,'minus')" class="basketPage-item__num-minus">-</div>
                    </div>
                    <div class="basketPage-item__price">{{item.price}} Р.</div>
                </div>
            </div>
            <div class="basketPage-finish">
                <div class="basketPage-counts">Количество: <span>{{count}} шт.</span></div>
                <div class="basketPage-total">Итого: <span>{{total}} Р.</span></div>
            </div>
        </div>
    ...
    export default {
        ...
        mounted() {
            if (localStorage.getItem('myProduct')) {
                ...
                this.count = JSON.parse(localStorage.getItem('myProduct')).count;
                this.total = JSON.parse(localStorage.getItem('myProduct')).total;
            }
        },
        methods: {
